...
public class GodFX_TransformInspector : Editor
{
	static public GodFX_TransformInspector instance;
	...
	/// 绘制坐标
	/// </summary>
	void DrawSomeComponent ()
	{
		GUILayout.BeginHorizontal();
		{
            /*
			if (GUILayout.Button ("动画", GUILayout.Width (33), GUILayout.Height (15))){
				Selection.activeTransform.gameObject.AddComponent<NcCurveAnimation>();

			}
			GUILayout.Space(-5);   //    在两个按钮间插入 像素
			if (GUILayout.Button ("UV", GUILayout.Width (33), GUILayout.Height (15))){
				Selection.activeTransform.gameObject.AddComponent<NcUvAnimation>();
			}
			GUILayout.Space(-5);   //    在两个按钮间插入 像素
			if (GUILayout.Button ("预制", GUILayout.Width (33), GUILayout.Height (15))){
				Selection.activeTransform.gameObject.AddComponent<NcAttachPrefab>();
			}
			GUILayout.Space(-5);   //    在两个按钮间插入 像素
             * * */
            if (Selection.activeTransform)  {
				ParticleSystem ps =	Selection.activeTransform.gameObject.GetComponent<ParticleSystem>();
				if(!ps)
				if (GUILayout.Button ("粒子", GUILayout.Width (33), GUILayout.Height (15))){
					Selection.activeTransform.gameObject.AddComponent<ParticleSystem>();
				}
			}
