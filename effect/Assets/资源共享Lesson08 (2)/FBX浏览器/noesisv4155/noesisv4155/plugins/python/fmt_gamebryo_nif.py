...
class NifObject:
	...
	def loadGenericString(self, bs):
		...
		if self.nif.fileVer < nifVersion(20, 1, 0, 1):
			...
			str = self.nif.stringTable[strIndex] if strIndex != 0xFFFFFFFF else ""
		return str
	...
	def loadAVObject(self, bs):
		...
		self.flags = bs.readUShort()
		if self.nif.nifHack == NIF_HACK_SKYRIM:
			bs.readUShort()
		translation = NoeVec3( (bs.readFloat(), bs.readFloat(), bs.readFloat()) )
	...
	def loadTimeController(self, bs):
		...
		#print("timecontroller nextid:", self.nif.objects[self.timeContNextID].name if self.timeContNextID != 0xFFFFFFFF else "none", "targetid:", self.nif.objects[self.timeContTargetID].name)
...
class NifFile:
	...
	def createNoeMatFromProperties(self, noeMatList, noeTexList, mesh):
		...
		if bsShaderProp is not None:
			...
			if bsShaderProp.bsTexSetID != 0xFFFFFFFF:
		...
		if texPropObj is not None and len(texPropObj.texList) > 0:
			...
			if tex0 is not None and tex0.sourceTexLinkID != 0xFFFFFFFF:
			...
			if tex6 is not None and tex6.sourceTexLinkID != 0xFFFFFFFF:
			...
			if tex3 is not None and tex3.sourceTexLinkID != 0xFFFFFFFF:
	...
	def rpgCommitTriangleGeometry(self, noeBones, bindCorrectionBones, noeMatList, noeTexList):
		...
		for geom in self.nifTriGeom:
			...
			if geom.geomSkinLinkID != 0xFFFFFFFF:
				skinInstance = self.objects[geom.geomSkinLinkID]
				if skinInstance.skinDataID != 0xFFFFFFFF:
			...
			if weightsPerVertex > 0:
				...
				noeBoneIndex = noeSafeGet(geom, "noeBoneIndex")
...
def nifConstructModelFromNif(nif):
	...
	if mdl is not None:
		...
		if nif.nifHack == NIF_HACK_OBLIVION or nif.nifHack == NIF_HACK_SKYRIM:
		...
		for seq in seqNif.nifSequences:
			...
			for evalID in seq.seqEvalIDList:
				...
				if objToEval is None:
					...
					elif evalObj.evalDataID != 0xFFFFFFFF:
